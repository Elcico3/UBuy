...
<jsp:setProperty name="beanModificaProdotto" property="*" />
...
%>
<%
	List<ProdottoBean> listaBean = pb.ottieniBeanProdottiVenditore();
	//System.out.println("nome prodotto : " + p.get(1).getNome());
	/*
%>
...
<%
	for (int i = 0; i < listaBean.size(); i++) {
		if (request.getParameter("btnModifica" + i) != null) {
			beanModificaProdotto.setNome(listaBean.get(i).getNome());
			beanModificaProdotto.setCategorie(listaBean.get(i).getCategorie());
			beanModificaProdotto.setPrezzo(listaBean.get(i).getPrezzo());
			beanModificaProdotto.setDescrizione(listaBean.get(i).getDescrizione());
			//beanModificaProdotto.stampaBean();
%>
<jsp:forward page="iniziaModifica.jsp" />
<%
	}
	}
%>
		...
		<div class="container">
			<div class="row">
				<div class="col-lg-12 text-center">
					<h2 class="section-heading">Modifica un prodotto</h2>
					<h3 class="section-subheading text-muted">Scegli il prodotto
						da modificare</h3>
				</div>
			</div>
			<div class="row">
				...
				%>
				<jsp:forward page="noProdotti.jsp"></jsp:forward>
				<%
					}
					for (int i = 0; i < listaBean.size(); i++) {
				%>
				<div class="col-sm-4">
					<div class="team-member">
						<form>
							<%
								if (listaBean.get(i).getCategorie().equals("Vestiario")) {
							%>
							<img src="img/prodotti/giacca225.jpg"
								class="img-fluid rounded-circle" alt="">
							<%
								}
							%>
							<%
								if (listaBean.get(i).getCategorie().equals("Casa")) {
							%>
							<img src="img/prodotti/casa225.jpg"
								class="img-fluid rounded-circle" alt="">
							<%
								}
							%>
							<%
								if (listaBean.get(i).getCategorie().equals("Libri/Cd")) {
							%>
							<img src="img/prodotti/icona_libro225.jpg"
								class="img-fluid rounded-circle" alt="">
							<%
								}
							%>

							<p style="color: white">Nome: <%=listaBean.get(i).getNome()%></p>
							<p style="color: white">Prezzo: <%=listaBean.get(i).getPrezzo()%> Euro</p>
							<p style="color: white">Categoria: <%=listaBean.get(i).getCategorie()%></p>
							<p class="text-muted"></p>

							<button type="submit" class="btn btn-xl" name="btnModifica<%=i%>">Modifica</button>
						</form>
					</div>
				</div>
				<%
